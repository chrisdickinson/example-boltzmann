...
let ajvLoose = null;
let ajvStrict = null;
async function main({ middleware = _requireOr('./middleware', []).then(_processMiddleware), bodyParsers = _requireOr('./body', [urlEncoded, json]), handlers = _requireOr('./handlers', {}), } = {}) {
...
async function router(handlers) {
    ...
    return function router(context) {
        ...
        const match = wayfinder.find(context.request.method, pathname, ...(context.request.headers['accept-version']
            ? [context.request.headers['accept-version']]
            : []));
        if (!match) {
            throw new NoMatchError(context.request.method, pathname);
        }
        const { method, route, decorators, middleware, version } = match.handler;
        context._routed = {
...
function session({ cookie = process.env.SESSION_ID || 'sid', secret = process.env.SESSION_SECRET, salt = process.env.SESSION_SALT, logger = bole('boltzmann:session'), load = 
// 
...
// 
iron = {}, cookieOptions = {}, expirySeconds = 60 * 60 * 24 * 365 } = {}) {
    let _iron = null;
...
// 
// 
function handleStatus({ git = process.env.GIT_COMMIT, reachability = {
// 
// 
...
function validateBody(schema) {
    ...
    return function validate(next) {
        return async (context, ...args) => {
            ...
            if (!valid) {
                context._body = Promise.reject(Object.assign(new Error('Bad request'), { errors: validator.errors, [STATUS]: 400 }));
                context._body.catch(() => { });
            }
            else {
...
let savepointId = 0;
function test({ middleware = [], handlers = _requireOr('./handlers'), bodyParsers = _requireOr('./body', [urlEncoded, json]), after = require('tap').teardown }) {
...
//# sourceMappingURL=boltzmann.js.map